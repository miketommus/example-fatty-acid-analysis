...
# Provide a mapping of the name of data files and the names of 
# relevant variables within them. 
# Below is the code that runs to combine all data files into a master dataframe. 

# Create lists of variables to pull from sample_list and gravimetry sheet
# format is names = "objects" & "objects" must match the colnames in your data
# do not modify names
sample_vars <- list(
    sample_ID = "sample_ID",
...
grav_vars <- list(
    sample_ID = "sample_ID",
    tiss_dry_mass = "tiss_dry_mass",
    int_std_conc = "int_std_conc",
...
source('data/example_ext_std_contents_tibble.R')

# Source the lookup table for GCMS response factors for instrument used.
source('data/example_rf_map_tibble.R')
...
# Reads in data files and renames columns as needed
sample_list <- xlsx_na_fix(files$sample_list) %>%
    rename(
        all_of(unlist(sample_vars))
    )
...
grav_results <- xlsx_na_fix(files$grav_results) %>%
    rename(
        all_of(unlist(grav_vars))
    )

# Standardizes sample_ID data type
sample_list$sample_ID <- as.character(sample_list$sample_ID)
gcms_results$sample_ID <- as.character(gcms_results$sample_ID)
grav_results$sample_ID <- as.character(grav_results$sample_ID)
...
long_conc$treatment %<>% factor(levels = treatments$treatment)
